...
* **dind**는 하나의 컨테이너 안에서 또 다른 여러 컨테이너들이 존재
  * 컨테이너를 실행하기 위해서 이미지를 전달할 수 있는 컨테이너를 만들어야 함(registry) **p109**
* 우리는 여러개의 가상머신(노드)을 연결해서(스웜) 각 노드마다 있는 여러 개의 복사 된 컨테이너들을 관리

![dind와 swarm 차이](C:\dev\TIL\2020_09_17\dind와 swarm 차이.PNG)
...
![image-20200917134434523](https://github.com/nickhealthy/TIL/blob/master/2020_09_17/img/1.%20%EB%8F%84%EC%BB%A4%20%EB%B9%84%EC%A3%BC%EC%96%BC%ED%99%94%2C%20dind.PNG)
...
##### 스웜 모드는 `secret`과 `config` 기능을 제공

* 스웜 모드와 같은 서버 클러스터에서 파일 공유를 위해 설정 파일을 호스트마다 마련해두는 것은 매우 비효율적임
  * **단일 호스트**에서는 `volume`이나 `-e` 옵션을 통해 디텍토리 공유를 모든 컨테이너에 공유가 가능했지만, 클러스터같이 **멀티 호스트들**을 각각의 호스트마다 `volume`으로 관리하는 것은 비효율적!
...
2. 생성한 secret을 통해 MySQL 컨테이너 생성
   ...
   * `--secret source`=my_mysql_password,`target`=mysql_password 처럼 디렉터리 지정(생성) 가능
...
9bbujnr412d4up4bxwrrr1xlu   `registry-config`     7 seconds ago       7 seconds ago
```

3. config는 입력된 값을 BASE64로 인코딩해서 저장
...
[
    {
        "ID": "9bbujnr412d4up4bxwrrr1xlu",
        "Version": {
            "Index": 908
        },
        "CreatedAt": "2020-09-17T18:31:26.593346285Z",
        "UpdatedAt": "2020-09-17T18:31:26.593346285Z",
        "Spec": {
            "Name": "registry-config",
            "Labels": {},
            "Data": `""`
        }
    }
]
```

4. BASE64로 디코딩해서 입력된 값 확인

```bash
root@swarm-manager:~# echo dmVyc2lvbjogMC4x ... jogMC4wLjAuMDo1MDAwCg== | base64 -d
version: 0.1
```

* BASE64 인코딩 : 눈에 보이도록 **가시화**를 해줌
  * 인코딩 : 규칙에 따라 데이터를 다른 형태로 변형
  * BASE64 디코딩 : 이진수로 바꿔서 6비트 단위로 짤라서 규칙에 따라서 바꾼다.
  * BASE64 인코딩 : 0~63(6비트) `->` 영문자(대문자, 소문자), 숫자, +, / 글자로 변경

![2.base64](https://github.com/nickhealthy/TIL/blob/master/2020_09_17/img/2.base64.PNG)

5. config로 사설 레지스트리 생성

```bash
root@swarm-manager:~# docker service create --name yml_registry -p 5000:5000 --config
```
...
![네트워크 1](https://github.com/nickhealthy/TIL/blob/master/2020_09_17/img/%EB%84%A4%ED%8A%B8%EC%9B%8C%ED%81%AC%201.PNG)

![네트워크 2](https://github.com/nickhealthy/TIL/blob/master/2020_09_17/img/%EB%84%A4%ED%8A%B8%EC%9B%8C%ED%81%AC%202.PNG)

